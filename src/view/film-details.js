...
export default class FilmDetails extends Abstract {
  ...
  renderNewComment() {
    this._newCommentComponent = new NewCommentView(this._newComment);
    render(this.getElement().querySelector(`.film-details__comments-wrap`), this._newCommentComponent);
  }
  _handleEmojiChange(emojiName) {
    this._newCommentComponent.updateData({emoji: emojiName});
  }
